...
class API
{
    ...
    public function RequireLogin($scope = [])
    {
        ...
        $accessTokenSet = request()->session()->has(self::SessionAccessTokenStore);
        $scopeSet = request()->session()->has(self::SessionScopeStore);
        $scopeMatch = true;

        if ($scopeSet) {
            foreach ($scope as $requestedScope) {
                if (!in_array($requestedScope, request()->session()->get(self::SessionScopeStore))) {
                    $scopeMatch = false;
                }
            }
        }
        ...
        if (!($accessTokenSet && $scopeSet && $scopeMatch))
        {
            ...
            $responseFn = function($request) use($requestURL, $prevScope, $scope) {
                $request->session()->put(self::SessionStateStore, $requestURL->State);
                $request->session()->put(self::SessionRequestedScopeStore, array_merge($prevScope, $scope));
                $request->session()->save();
                return redirect()->to($requestURL->URL);
            };

            $responseFn($request)->send();exit;
        }
    ...
    private function TryDoLogin()
    {
        if (!$this->hasTriedLogin && array_key_exists(self::UriDoLoginKey, $_GET) && $_GET[self::UriDoLoginKey] === self::UriDoLoginValue) {
            ...
            $expectedState = request()->session()->get(self::SessionStateStore, null);
            ...
            try {
                ...
                // Access Token
                request()->session()->put(self::SessionAccessTokenStore, $accessToken);

                // Requested scopes
                if (request()->session()->has(self::SessionRequestedScopeStore)) {
                    request()->session()->put(self::SessionScopeStore, request()->session()->get(self::SessionRequestedScopeStore));
                }

                // Remove old state
                request()->session()->forget(self::SessionStateStore);
